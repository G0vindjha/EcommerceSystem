...
class Connection
{
    ...
    public function __construct()
    {
        try {
            ...
            $this->conn = new PDO("mysql:host=192.168.102.241:3306;dbname=eCommerce", 'root', 'deep70');
    ...
    public function delete($table, $where, $whereParam, $image='yes')
    {
        if ($image == "no") {
            ...
            if ($table == 'Customers') {
                $imgpath = '/var/www/html/ops/public_html/PHPOPS/eCommerce/assets/image/customerUpload/' . $value['value'] . '/' . $profile_Pic;
                $dirpath = '/var/www/html/ops/public_html/PHPOPS/eCommerce/assets/image/customerUpload/' . $value['value'];
            } else {
                $imgpath = '/var/www/html/ops/public_html/PHPOPS/eCommerce/assets/image/productUpload/' . $value['value'] . '/' . $profile_Pic;
                $dirpath = '/var/www/html/ops/public_html/PHPOPS/eCommerce/assets/image/productUpload/' . $value['value'];
            }
            $sql = "DELETE FROM $table WHERE $where";
            $stmt = $this->conn->prepare($sql);
            foreach ($whereParam as $key => $value) {
