...
if ($_POST['action'] == 'searchData') {
    ...
    $new = array_filter($result, function ($value) use ($search, &$srno) {
        ...
        if (strpos(strtolower($value['name']), strtolower($search)) !== FALSE || strpos(strtolower($value['email']), strtolower($search)) !== FALSE) {
            $srno++;
            $imgpath = SITE_URL . 'PHPOPS/eCommerce/assets/image/customerUpload/' . $value['customer_id'] . '/' . $value['image'];

            $action = "<div class='row d-flex flex-nowrap'>
        <div class='col'><a href='" . SITE_URL . "PHPOPS/eCommerce/admin/addCustomer.php?customer_id="  . $value['customer_id'] . "' type='button' id='udp"  . $value['customer_id'] . "' class='udp btn btn-success upd'>UPDATE</a></div>
        ...
        <main class='position-relative my-3'>
            <div class="row d-flex justify-content-between mb-3">
                ...
                <div class="col-lg-4 col-6 d-flex justify-content-end">
                    <a class="btn btn-outline-success" href="<?php echo SITE_URL; ?>PHPOPS/eCommerce/admin/addCustomer.php" role="button">Add Customer</a>
                </div>
                <!-- customer list tabel -->
